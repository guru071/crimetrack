...
import { ShieldAlert, Send, AlertTriangle, FileText, X, User, Edit2, Trash2, Check, LogOut, Search, Camera, Plus, Shield } from 'lucide-react';
...
export default function OperationsRoom({ currentUser, profile, records = [], getHumanModel, onAddRecord, onDeleteRecord }) {
  ...
  // Quick-add criminal record form
  const [showAddRecord, setShowAddRecord] = useState(false);
  const [newRecord, setNewRecord] = useState({ name: '', firNumber: '', status: 'Active', age: '', sex: 'Male', areaOfOperation: '' });
    ...
    conn.on('data', (data) => {
      ...
      if (data.type === 'record_deleted') {
        const systemMsg = {
          id: crypto.randomUUID(),
          type: 'system',
          text: `🗑️ ${data.officerName} deleted record: ${data.recordName}`,
          senderId: 'system',
          senderName: 'System',
          timestamp: new Date().toLocaleTimeString()
        };
      ...
      if (data.type === 'chat' || data.type === 'record_share') {
        let decryptedText = '[Encrypted Data - Wrong Key]';
        try {
          const bytes = CryptoJS.AES.decrypt(data.text, password);
          const t = bytes.toString(CryptoJS.enc.Utf8);
  ...
  // ─── Add Criminal Record from Operations Room ──────────────────────────────
  const handleAddRecord = async () => {
    ...
    try {
      const rec = {
        ...newRecord,
        age: newRecord.age ? Number(newRecord.age) : '',
        id: crypto.randomUUID(),
        createdAt: new Date().toISOString(),
        addedFromOperation: roomId,
        addedBy: profile?.name || currentUser.email
      };
      if (onAddRecord) await onAddRecord(rec);
      ...
      setMessages(prev => [...prev, {
        ...broadcastObj,
        text: `📋 You added a new record: ${rec.name}`,
        type: 'system',
        senderName: 'System'
      }]);

      setNewRecord({ name: '', firNumber: '', status: 'Active', age: '', sex: 'Male', areaOfOperation: '' });
  ...
  const handleDeleteRecord = (recordId, recordName) => {
    ...
    setMessages(prev => [...prev, {
      ...broadcastObj,
      text: `🗑️ You deleted record: ${recordName}`,
      type: 'system',
      senderName: 'System'
    }]);
        ...
        <div style={{ padding: '72px 14px', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100dvh' }}>
          <div style={{ width: '100%', maxWidth: '100%', background: 'transparent', padding: 24, borderRadius: 24, border: '1px solid rgba(239,68,68,0.2)' }}>
            ...
            <form onSubmit={joinRoom} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <input type="password" style={{ padding: '16px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, color: 'var(--ct-text)', fontSize: 16 }} placeholder="Secret Passkey" value={password} onChange={e => setPassword(e.target.value)} required autoFocus />
              {password && <div style={{ fontSize: 11, color: '#10b981', textAlign: 'center' }}>🔑 Password auto-filled from Firebase</div>}
              <button type="submit" style={{ padding: '16px', background: '#ef4444', border: 'none', borderRadius: 12, color: 'var(--ct-text)', fontWeight: 'bold', fontSize: 16, cursor: 'pointer' }}>Connect Securely</button>
              <button type="button" onClick={() => setViewState('history')} style={{ padding: '16px', background: 'transparent', border: 'none', color: 'var(--ct-muted)', cursor: 'pointer', fontSize: 15 }}>Cancel</button>
            </form>
          </div>
        </div>
      );
    ...
    return (
      <div style={{ padding: '72px 14px', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100dvh' }}>
        <div style={{ width: '100%', maxWidth: '100%', background: 'transparent', padding: 24, borderRadius: 24, border: '1px solid rgba(239,68,68,0.2)' }}>
          ...
          <form onSubmit={joinRoom} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <input style={{ padding: '16px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, color: 'var(--ct-text)', width: '100%', boxSizing: 'border-box' }} placeholder="Operation ID (e.g. ALPHA-1)" value={roomId} onChange={e => setRoomId(e.target.value)} required />
            <input style={{ padding: '16px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, color: 'var(--ct-text)', width: '100%', boxSizing: 'border-box' }} placeholder="Secret Passkey" type="password" value={password} onChange={e => setPassword(e.target.value)} required />
            <button type="submit" style={{ padding: '16px', background: '#ef4444', border: 'none', borderRadius: 12, color: 'var(--ct-text)', fontWeight: 800, cursor: 'pointer' }}>
              Connect to Secure Channel
            </button>
            {history.length > 0 && (
              <button type="button" onClick={() => setViewState('history')} style={{ marginTop: 8, padding: '16px', background: 'transparent', border: 'none', color: 'var(--ct-muted)', width: '100%', cursor: 'pointer' }}>
  ...
  // ─── IN-ROOM CHAT VIEW ─────────────────────────────────────────────────────
  return (
    <div style={{ padding: '72px 14px 100px', display: 'flex', flexDirection: 'column', height: '100dvh', boxSizing: 'border-box' }}>
      {/* Header */}
          ...
          return (
            <div key={msg.id} style={{ alignSelf: isMe ? 'flex-end' : 'flex-start', maxWidth: '85%' }}>
              ...
              {editingMsgId === msg.id ? (
                ...
                <>
                  {isRecord && parsedRecord ? (
                    <div onClick={() => setSelectedRecordToView(parsedRecord)} style={{ background: 'rgba(59,130,246,0.1)', border: '1px solid #3b82f6', padding: '10px', borderRadius: 12, cursor: 'pointer', display: 'flex', gap: 10, alignItems: 'center' }}>
                      {parsedRecord.photo ? (
                        <img src={parsedRecord.photo} style={{ width: 40, height: 40, borderRadius: 20, objectFit: 'cover' }} />
                      ) : (
                        <div style={{ width: 40, height: 40, borderRadius: 20, background: 'color-mix(in srgb, var(--ct-text) 10%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><User size={20} /></div>
                      )}
                      <div>
                        <div style={{ color: '#3b82f6', fontWeight: 'bold', fontSize: 14 }}>{parsedRecord.name}</div>
                        <div style={{ fontSize: 11, color: 'var(--ct-text)' }}>Shared Record. Tap to view.</div>
                      </div>
                    </div>
                  ) : (
              ...
              <div style={{ display: 'flex', justifyContent: isMe ? 'flex-end' : 'flex-start', alignItems: 'center', gap: 12, marginTop: 4, padding: '0 4px' }}>
                ...
                {isMe && !isRecord && editingMsgId !== msg.id && (
      ...
      {/* Input Bar */}
      <form onSubmit={sendMessage} style={{ display: 'flex', gap: 8, position: 'fixed', bottom: 70, left: 14, right: 14, zIndex: 100 }}>
        <button type="button" onClick={() => setShowRecordPicker(true)} style={{ width: 48, height: 48, borderRadius: 24, background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', color: 'var(--ct-text)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }} title="Share existing record">
          <FileText size={18} />
        </button>
        <button type="button" onClick={() => setShowAddRecord(true)} style={{ width: 48, height: 48, borderRadius: 24, background: 'rgba(16,185,129,0.15)', border: '1px solid rgba(16,185,129,0.4)', color: '#10b981', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }} title="Add new criminal record">
          <Plus size={20} />
        </button>
      ...
      {showAddRecord && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.92)', backdropFilter: 'blur(10px)', zIndex: 999999, display: 'flex', flexDirection: 'column', padding: 20, overflowY: 'auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, marginTop: 40 }}>
            <div style={{ color: '#10b981', fontWeight: 'bold', fontSize: 18, display: 'flex', alignItems: 'center', gap: 8 }}>
              <Plus size={20} /> Add Criminal Record
            </div>
            <X color="var(--ct-text)" size={24} onClick={() => setShowAddRecord(false)} style={{ cursor: 'pointer' }} />
          </div>
          <div style={{ background: 'var(--ct-card)', borderRadius: 16, padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[
              { key: 'name', label: 'Full Name *', placeholder: 'e.g. Rajesh Kumar' },
              { key: 'firNumber', label: 'FIR Number', placeholder: 'e.g. FIR/2024/001' },
              { key: 'age', label: 'Age', placeholder: 'e.g. 32', type: 'number' },
              { key: 'areaOfOperation', label: 'Area of Operation', placeholder: 'e.g. North Goa' },
            ].map(f => (
              <div key={f.key}>
                <div style={{ fontSize: 11, color: 'var(--ct-muted)', textTransform: 'uppercase', marginBottom: 6 }}>{f.label}</div>
                <input
                  type={f.type || 'text'}
                  placeholder={f.placeholder}
                  value={newRecord[f.key]}
                  onChange={e => setNewRecord(r => ({ ...r, [f.key]: e.target.value }))}
                  style={{ width: '100%', padding: '12px 14px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, color: 'var(--ct-text)', boxSizing: 'border-box', fontSize: 14 }}
                />
              </div>
            ))}
            <div style={{ display: 'flex', gap: 12 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 11, color: 'var(--ct-muted)', textTransform: 'uppercase', marginBottom: 6 }}>Status</div>
                <select value={newRecord.status} onChange={e => setNewRecord(r => ({ ...r, status: e.target.value }))} style={{ width: '100%', padding: '12px 14px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, color: 'var(--ct-text)', fontSize: 14 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 11, color: 'var(--ct-muted)', textTransform: 'uppercase', marginBottom: 6 }}>Sex</div>
                <select value={newRecord.sex} onChange={e => setNewRecord(r => ({ ...r, sex: e.target.value }))} style={{ width: '100%', padding: '12px 14px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, color: 'var(--ct-text)', fontSize: 14 }}>
                  <option>Male</option>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 4 }}>
              <button type="button" onClick={() => setShowAddRecord(false)} style={{ flex: 1, padding: 14, borderRadius: 12, background: 'rgba(255,255,255,0.05)', border: 'none', color: 'var(--ct-text)', fontWeight: 'bold', cursor: 'pointer' }}>Cancel</button>
              <button type="button" onClick={handleAddRecord} disabled={addingRecord || !newRecord.name.trim()} style={{ flex: 1, padding: 14, borderRadius: 12, background: '#10b981', border: 'none', color: '#fff', fontWeight: 'bold', cursor: addingRecord || !newRecord.name.trim() ? 'not-allowed' : 'pointer', opacity: addingRecord || !newRecord.name.trim() ? 0.6 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                <Plus size={16} /> {addingRecord ? 'Saving...' : 'Add Record'}
              </button>
            </div>
          </div>
        </div>
      ...
      {isScanning && (
        <FaceScannerModal records={records} getHumanModel={getHumanModel} onClose={() => setIsScanning(false)} onMatch={(matchedRecord) => { setIsScanning(false); setRecordToConfirm(matchedRecord); }} />
      )}
